...
class _ProviderListViewState extends State<ProviderListView> {
  ...
  void initState() {
    ...
    filteredProfiles = [];
    // Simulate a delay to show the CircularProgressIndicator
      ...
      builder: (context, profiles) {
        ...
        if (widget.searchQuery != null && widget.searchQuery!.isNotEmpty) {
          filteredProfiles = profiles.where((profile) {
            ...
            return profile.name!.toLowerCase().contains(widget.searchQuery!.toLowerCase());
          }).toList();
        }
  ...
  Widget _buildListView() {
    if (showLoading) {
      ...
      if (filteredProfiles.isNotEmpty) {
        ...
        return const Center(child: Text("No Photographers with this name",style: TextStyle(color: Colors.black , fontSize: 24 , fontWeight: FontWeight.bold),));
      }
    }
  }
}
